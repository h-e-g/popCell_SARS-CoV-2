...
COUNT_DATA_FILE=sprintf('%s/Counts_and_CPM__pseudoBulk_full_%slibs__per_%s_IID_and_LIB.tsv.gz',DATA_DIR,NLIBS,STATE)
...
MeanCPM=Counts[,.(meanCPM=mean(CPM)),by=.(ID,Symbol,state)]
expressed_genes=MeanCPM[meanCPM>1,.N,by=ID][,ID]

# write down this list
fwrite(data.table(expressed_genes),file=sprintf('%s/expressed_genes_CPMover1_%slibs__per_%s.tsv',OUT_DIR, NLIBS, STATE),sep='\t')
...
tim=Sys.time()
Prov=Counts[ID%in%expressed_genes,explainVar_estimate_IIDeffect(CPM, IID, LIB, p3_date, flowcellID, logged=TRUE, test_ID=paste(state ,ID ,Symbol,sep='_')),by=.(state,ID,Symbol)]
...
fwrite(Var_estimates,file=sprintf('%s/VarianceExplained_by_IID_and_batch_%slibs__per_%s.tsv.gz',OUT_DIR,NLIBS,STATE),sep='\t')
...
fwrite(BatchAdjusted_logCPM,file=sprintf('%s/BatchAdjusted_logCPM_%slibs__per_%s.tsv.gz',OUT_DIR,NLIBS,STATE),sep='\t')


tic('adding cell count information and meta_data to main table')
#### compute cellcounts per cluster/IID
Cells_per_sample=Counts[,.(ncells=sum(ncells)),by=.(state,IID,ID)]

#### add cellcounts to main table
BatchAdjusted_logCPM=merge(BatchAdjusted_logCPM, Cells_per_sample,by=c('state','ID','IID'))
...
fwrite(BatchAdjusted_logCPM,file=sprintf('%s/BatchAdjusted_logCPM_%slibs__per_%s_annotated.tsv.gz',OUT_DIR,NLIBS,STATE),sep='\t')
toc()
