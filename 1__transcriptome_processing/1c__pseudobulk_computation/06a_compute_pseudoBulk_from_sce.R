...
# Pct cluster by celltype, IID, state
...
saveRDS(summed,file=sprintf('%s/sce_pseudoBulk_full_%slibs_per_%s_%s_IID_LIB.RDS',OUT_DIR,NLIBS,CELLTYPE,STATE))
...
fwrite(Counts,file=sprintf('%s/Counts_and_CPM__pseudoBulk_full_%slibs__per_%s_%s_IID_and_LIB.tsv.gz',OUT_DIR, NLIBS, CELLTYPE, STATE))
