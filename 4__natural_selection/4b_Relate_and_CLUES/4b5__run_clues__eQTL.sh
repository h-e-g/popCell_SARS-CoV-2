...
################################################################################
################################################################################
# File name: 4b5__run_clues__eQTL.sh
# Author: Y.A., M.R., M.ON.
################################################################################
################################################################################
...
# SLURM options
#SBATCH --job-name=4b5
#SBATCH --output=${LOG_DIR}/4b5__%J.log
#SBATCH --mem=50G
...
echo "Re-sampling branch lengths. ${CHR}:${POS} (${RSID}) in ${POP} (eQTL, CutOff=${CUTOFF})."
...
if test -f "${OUT_DIR}/${POP}_CutOff${CUTOFF}/SNPs/${EXPRESP}/${RSID}/relate_samplebranch_${RSID}.timeb"; then
  ...
  cd ${IN_DIR}/${POP}
  mkdir -p ${OUT_DIR}/${POP}_CutOff${CUTOFF}/SNPs/eQTL/${RSID}
...
cd ${OUT_DIR}/${POP}_CutOff${CUTOFF}/SNPs/eQTL/${RSID}
